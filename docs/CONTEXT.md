...
**Cascade Step**
One ordered entry in a Cascade, targeting a user Envelope with a funding type. Cinco tipos: `fixed` (a fixed USD amount, **accumulates** month over month), `fixedUntilCap` (a fixed USD amount, but capped so it never pushes the destination Envelope past its `Cap`; with no `Cap`, degrades to `fixed`), `fill-to-cap` (tops up only to the Envelope's `Cap`, does **not** accumulate), `percentOfRemainder` (a percentage of the remaining — or gross — amount), or `catchAll` (everything left; at most one, must be last). The funding type is **independent** of the Envelope's Funding Target marker. See [ADR-0015](adr/ADR-0015-cascada-distribucion.md) §C2-4/§C2-7/§C2-11.
