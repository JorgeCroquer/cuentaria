...
void main() {
  group('RegistrarReverso', () {
    ...
    test('reversar una transacción simple invierte los postings exactamente', () async {
    ...
    test('reversar una realización revierte su diferencial (3 postings)', () async {
      final cuentaId = AccountId('acc-bs');
      final sobreGastoId = EnvelopeId('env-gasto');
      final diferencialId = catalog.getSystemEnvelope(EnvelopeRole.diferencial);
      ...
      final postingsOrig = [
        Posting(
          target: CuentaTarget(cuentaId),
          amountNative: Money(amount: BigInt.from(-100000), currency: CurrencyCode('VES')),
        ...
        Posting(
          ...
          amountNative: Money(amount: BigInt.from(-2000), currency: CurrencyCode('USD')),
          currency: CurrencyCode('USD'),
