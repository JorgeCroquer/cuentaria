...
void main() {
  group('ReconstruirProyecciones', () {
    test('limpia y reconstruye idéntico desde el log de eventos', () async {
      ...
      final saldoSobreReconstruido = projections.saldoUsdSobre(
        EnvelopeId('env-1'),
      );

      expect(
        saldoUsdReconstruido.native.amount,
        equals(saldoUsdOriginal.native.amount),
      );
    ...
    test(
      'Property Test: auto-heal y estado idéntico tras replay con eventos retrofechados',
      () async {
        final store = InMemoryEventStore();
