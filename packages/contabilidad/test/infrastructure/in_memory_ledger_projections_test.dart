...
void main() {
  group('InMemoryLedgerProjections', () {
    ...
    test('Property Test: Extracción proporcional de costo base y residuo cero (C1-6)', () {
      final projections = InMemoryLedgerProjections();
      ...
      projections.aplicar(Transaccion.crear(
        postings: [
          Posting(
            target: CuentaTarget(AccountId('acc-usd')),
            amountNative: Money(amount: BigInt.from(-10000), currency: CurrencyCode('USD')),
            currency: CurrencyCode('USD'),
            amountUsd: -10000,
          ),
          Posting(
            target: CuentaTarget(accBsId),
            amountNative: Money(amount: BigInt.from(400000), currency: CurrencyCode('VES')), // 4000.00
            currency: CurrencyCode('VES'),
      ...
      projections.aplicar(Transaccion.crear(
        postings: [
          Posting(
            target: CuentaTarget(AccountId('acc-usd')),
            amountNative: Money(amount: BigInt.from(-6000), currency: CurrencyCode('USD')),
            currency: CurrencyCode('USD'),
            amountUsd: -6000,
          ),
          Posting(
            target: CuentaTarget(accBsId),
            amountNative: Money(amount: BigInt.from(200000), currency: CurrencyCode('VES')), // 2000.00
            currency: CurrencyCode('VES'),
      ...
      expect(saldoTotal.native.amount, equals(BigInt.from(600000))); // 6000.00 Bs
      ...
      final saldoDespues1 = SaldoCuenta(
        native: Money(amount: saldoTotal.native.amount - BigInt.from(150000), currency: CurrencyCode('VES')),
      ...
      final saldoDespues2 = SaldoCuenta(
        native: Money(amount: saldoDespues1.native.amount - BigInt.from(300000), currency: CurrencyCode('VES')),
        usd: saldoDespues1.usd - costo2Real,
      );
      
      // Retiro final (todo el remanente)
      final costo3Real = saldoDespues2.costoBaseDe(saldoDespues2.native.amount); // C1-6 Zero-Native
