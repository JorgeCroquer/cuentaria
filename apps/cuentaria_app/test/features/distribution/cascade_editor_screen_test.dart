...
void main() {
  ...
  testWidgets(
    'a fixedUntilCap step can be added and shows the derived amount (#302)',
    (tester) async {
      ...
      expect(find.text('Fijo hasta tope → Mercado'), findsOneWidget);
      expect(find.byKey(const Key('cascadeValidationErrors')), findsNothing);

      final fixedUntilCapTile = tester.widget<ListTile>(
        find.byKey(const Key('cascadeStep_0')),
      );
      expect(
        fixedUntilCapTile.subtitle,
        isA<SignedAmountText>().having(
          (w) => w.amount,
          'amount',
          '\$50.00 · hasta el tope del sobre',
        ),
      );
